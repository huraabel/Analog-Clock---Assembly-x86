...
if NOT exist "%file%.asm" (
    ...
    echo va asambla file.asm in file.obj si apoi va linkedita file.obj in file.exe
    exit /B 1
)

del "%file%.obj" "%file%.exe" >nul 2>nul


call "%cale%ml.exe" "%file%.asm" /link /subsystem:console /entry:start "%cale%msvcrt.lib"
